<?php
	...
	if(isset($_POST['btnSaveStatus'])) {
		if($objFunc->qUpdateDestaqueStatus($_POST)) {
	...
	if(isset($_GET['action'])) {
		switch($_GET['action']) {
			case 'status': 
				$status = $objFunc->qSelectDestaque($_GET['id']); 
				break;
			case 'edit': 
				$dest = $objFunc->qSelectDestaque($_GET['id']); 
				break;
			case 'delet':
...
?>
		...
		<!-- inicio lista destaques section -->
					...
					<div class="col-md-10">
						<table class="table table-striped">
							...
							<tbody>
								<?php
									$i = 1;
									foreach($objFunc->qSelectAllDestaques() as $result) { ?>
										<tr>
											<th scope="row"><?=$i;?></th>
											<td><?=($result['name']); ?></td>
						...
						<div class="modal-dialog modal-md" role="document">
							<div class="modal-content">
								<div class="modal-header">
									...
									<h5 class="modal-title text-center" id="mod_contato">Alterar status do destaque</h5>
								</div>
								<div class="modal-body table-responsive">
									<div class="col-md-12">
										<form class="form-horizontal" action="<?php $_SERVER["PHP_SELF"];?>" method="post">
											...
											?>
											<p>O destaque "<i><?=($status['name']);?></i>" está atualmente <code><?=$st;?></code>.</p>
											<p>Deseja <u><?=$st_c;?></u>?</p>
											...
											<!--<div class="form-group">
												...
												<div class="col-sm-10">
													<input type="text" class="form-control" id="inputStatus" placeholder="Status" 
														name="activation" value="<?//=$dest['activation']; ?>" required >
												</div>
											</div>-->
